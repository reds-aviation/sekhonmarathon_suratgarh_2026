...
Do not publish assumed routes, maps or reporting times. Once the route is approved, publish a concise timeline with start/reporting point, control points, hydration/medical points, distance markers, turnaround or finish, and safety instructions. The public guide may link to it; only an AAL2 organiser with `route_publisher` or `event_admin` should be able to change it.
...
1. Apply migrations `001` through `014` in a staging Supabase project, in order. Execute access-control, correction, T-shirt and completion tests with separate participant and organiser accounts.
...
7. Rehearse phone and laptop flows: sign-in, invitation, registration, screenshot upload, rejected-payment correction, private proof access, review, T-shirt collection/void recovery and audit inspection.
8. Rehearse clocks, finish capture, result correction, certificate verification and the paper/manual backup with synthetic staging data. Do not use production clocks for rehearsal; leave completion disabled until the rehearsal is signed off.
9. Create a private Sheet and private receipt folder in the organiser Drive. Set `SPREADSHEET_ID`, `RECEIPTS_FOLDER_ID`, `SUPABASE_DRIVE_ENDPOINT` and the matching `DRIVE_MIRROR_HMAC_SECRET` as Apps Script properties, then enable the scheduled one-way import only after its export, access controls and recovery behaviour are verified.
